...
export const scanAndProcessExistingLivePhotos = async (): Promise<{
  ...
  try {
    ...
    logger.chrono.info('Scan completed. Use processSpecificLivePhotoVideo for individual files.')
...
export const processSpecificLivePhotoVideo = async (
  ...
  try {
    ...
    if (!isLivePhotoVideo(fileName, fileBuffer.length)) {
      logger.chrono.warn(`File does not appear to be a LivePhoto video: ${videoKey}`)
      return false
    }
    ...
    logger.chrono.error(`Failed to process specific LivePhoto video ${videoKey}:`, error)
    return false
  }
}
